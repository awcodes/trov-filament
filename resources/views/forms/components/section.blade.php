<div @if ($isCollapsible()) x-data="{ isCollapsed: {{ $isCollapsed() ? 'true' : 'false' }} }"
    ...
    id="{{ $getId() }}"
    {{ $attributes->merge($getExtraAttributes())->class(['p-6 space-y-6 rounded-xl shadow-sm border border-gray-300 bg-gray-50  filament-forms-section-component','dark:border-gray-800 dark:bg-gray-800/50' => config('forms.dark_mode')]) }}
    ...
    <div
        @if ($isCollapsible()) x-show="! isCollapsed"
